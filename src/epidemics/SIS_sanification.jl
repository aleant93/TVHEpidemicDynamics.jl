...
- `Δ`, time within which an indirect contact may happen. Used to compute the
intervals when sanite all locations;
    ...
    open(output_path, "a") do fhandle
        write(
            ...
            string(
                ...
                "sanitized,lockdown,imm_start,nodes_imm_strategy,hes_imm_strategy,",
        ...
        for iter=1:niter
            ...
            for t=1:length(intervals)
                ...
                to_write = string(
                    ...
                    "$(kwargs[:sanitize]),$(lockdown),$(imm_start),$(nodes_imm_strategy),$(hes_imm_strategy),",
                ...
                # All locations are sanitized
                # at midday
                if haskey(kwargs, :sanitize) && kwargs[:sanitize]
                    if t % (convert(Int, 24 / Δ)) == 1
