...
"""
    Experiments about epidemic blocking strategies using contact tracing
"""
...
fparams =
    joinpath(project_path, "experiments", "immunization", "configs", "tracing.json")

#
#FOURSQUARE
#
output_path = joinpath(project_path, "experiments", "immunization", "foursquare", "results")
...
for testtype in keys(test_data)
    for (index, test) in enumerate(get(test_data, testtype, nothing))
        ...
        # immunization params
        app_strategy =
            ismissing(test[:app_strategy]) ?
                nothing : getfield(Main, Symbol(test[:app_strategy]))
        ...
        nodes_kwargs =
            ismissing(test[:nodes_precomputed]) ? Dict{}() : Dict{}(:path => test[:nodes_precomputed])
        hes_kwargs =
            ismissing(test[:hes_precomputed]) ? Dict{}() : Dict{}(:path => test[:hes_precomputed])

        res_path =
            joinpath(output_path, "csv", "$(test[:exp_id])_$(test[:exp])_$(Dates.format(now(), "Y-mm-ddTHH-MM-SS")).csv")


        tracing_params = Dict{}(:app_strategy => app_strategy, :αᵢ => test[:αᵢ])
