...
function simulate(
    ...
    open(output_path, "a") do fhandle
        ...
        for iter=1:niter
            ...
            agents_per_day = kwargs[:people_per_day]
            n_swabs = kwargs[:n_swabs]
            ...
            for t=1:length(intervals)
                ...
                if t % (ceil(Int, 24/Δ)) == 1
                    ...
                    # select all people checking-in that day
                    agents = agents_per_day[day]

                    # consider only the agents not in quarantine
                    agents_to_check = [user2vertex[u] for u in agents if quarantine[user2vertex[u]] == 0]
                    agents_swabs = sample(agents_to_check, min(n_swabs, length(agents_to_check)))

                    for u in agents_swabs
                        if _vstatus[u] == 1
                            quarantine[u] = 1
                        end
                    end
